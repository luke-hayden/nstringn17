...
o.t.result <- apply(mkcoy, 1, function (x) t.test(x[1:7],x[8:15]))
mkc$p_value <- unlist(lapply(t.result, function(x) x$p.value))
mkc$t_stat <- unlist(lapply(t.result, function(x) x$statistic))


ctg34 <- as.data.frame(t(subset(t(ctg1to9), sampleinfo$ctg %in% c(3,4))))
ctg34 <- ctg34[order(rownames(ctg34)), ]


prop <- ctg34/bc[,1:24]


plot(log(melt(ctg34)$value), log(melt(bc)$value), pch=".")

hist(melt(ctg34)$value /melt(bc)$value)





bc2 <- bc[,c(2,11:14, 23:24, 3,4, 9,10, 15,16, 21,22)]


o.t.result <- apply(bc2, 1, function (x) t.test(x[1:7],x[8:15]))
o.pval <- unlist(lapply(o.t.result, function(x) x$p.value))

olds <- ctg34[,c(2,11:14, 23:24)]
youngs <- ctg34[, c(3,4,9,10, 15,16, 21,22)]

oys <- cbind(olds, youngs)



x.t.result <- apply(oys, 1, function (x) t.test(x[1:ncol(olds)],x[ncol(olds)+1: ncol(oys)]))

x.pval <- unlist(lapply(x.t.result, function(x) x$p.value))




x.t.result <- apply(oys, 1, function (x) t.test(x[1:7],x[8:15]))
...
ctg342 <- ctg34[,c(2,11:14, 23:24, 3,4, 9,10, 15,16, 21,22)]




n.t.result <- apply(ctg342, 1, function (x) t.test(x[1:7],x[8:15]))

n.pval <- unlist(lapply(n.t.result, function(x) x$p.value))


plot(n.pval, x.pval, pch=".")
...
x.t.result <- apply(oys, 1, function (x) t.test(x[1:ncol(old)],x[ncol(old)+1: ncol(oys)]))

x.pval <- unlist(lapply(x.t.result, function(x) x$p.value))

mdf <- as.data.frame(md)
mdf$name <- rownames(mdf)
mdf$pvaloy <- x.pval
mdf$oysig <- mdf$pvaloy < 0.05
...
bcmin <- ctg1to9[,log== TRUE]
bcmin.groups <- subset(sampleinfo$type, log==TRUE)
bcmin <- subset(bcmin, rownames(bcmin) %in% nmd$name[nmd$p_val<0.05])
tbc <- t(bcmin)
tbc <- tbc[,colSums(tbc) > 0]
bc.pca <- prcomp(tbc,center = TRUE,scale. = TRUE) 
bc.groups <- bcmin.groups
