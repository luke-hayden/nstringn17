...
ggplot(data=nik, aes(x=YvOl2fc, y=ROvOl2fc, label=sname, fill=regsd,colour=good, shape=ovrsig))+
  geom_hline(yintercept=0, colour="black")+
  geom_vline(xintercept=0, colour="black")+
  theme_minimal()+
  xlim(-6,+6)+
...
#  geom_rect(inherit.aes=FALSE,xmin=0, ymin=0, xmax=10, ymax=10, fill="yellow", alpha=0.004)+
  ...
  theme(    plot.background = element_rect(fill = "transparent",colour = "transparent") )+
  coord_fixed(ratio=1)+
scale_fill_gradient2(high="red2", mid="white", low="blue", midpoint=2.5, name="Standard deviation \n amongst regenerated")+
  facet_wrap(~ovysig)+
#  geom_text(size=1.7, aes(alpha=good))+
  scale_colour_manual(values=c("white", "black"),labels=c("Rejuvenating \nmarkers", ""), breaks=c(TRUE, F) , name=NULL)+
