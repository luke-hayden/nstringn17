...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
md <- t(subset(t(ctg1to9), sampleinfo$sex == "F"& sampleinfo$qual == "ok"& sampleinfo$exp == "OvY" & sampleinfo$ctg>2& sampleinfo$type %in% c("O", "Y") ))
...
nmd$sp <- substr(nmd$p_value, 1,4)


mnmd <- melt(nmd, id.vars=c("p_value", "sname", "up", "dir", "sp", "sig"))
mnmd$age <- "old"
mnmd$age[mnmd$variable %in% colnames(young)] <- "young"


ggplot(mnmd, aes(x= sname, y=value, colour=age, label=sp)) +
  theme_bw()+
...
a=ggplot(data=nik, aes(x=YvOl2fc, y=ROvOl2fc, label=sname, colour=in61))+
