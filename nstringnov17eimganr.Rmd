...
a=ggbiplot(bc.pca,choices=1:2, obs.scale = 1, var.scale = 1, ellipse = FALSE, circle = TRUE, var.axes=FALSE, groups=bc.groups)+ggtitle("PCA of Old vs Young (females): all 195 markers")+
    theme_bw() +
  ...
  scale_x_continuous(breaks=c(-12,-8,-4,0,4,8,12,16))+
  scale_y_continuous(breaks=c(-8,-4,0,4,8))

#coord_fixed(ratio=0.611)


ggsave(plot=a, height=5,width=7,dpi=200, device="png", filename=paste("pcaovy195.png"), limitsize = FALSE,bg = "transparent")

ggsave(plot=a, height=5,width=7,dpi=200, device="pdf", filename=paste("pcaovy195.pdf"), limitsize = FALSE)
log <- sampleinfo$sex =="F" & sampleinfo$ctg %in% c(3,4,5,7) & sampleinfo$qual == "ok" & sampleinfo$reg == "" & sampleinfo$age != "N"
...
b=ggbiplot(bc.pca, obs.scale = 1, var.scale = 1, ellipse = FALSE, circle = TRUE, var.axes=FALSE, groups=bc.groups)+ggtitle("PCA of Old vs Young (females): 61 chosen markers")+
    theme_bw() +
  scale_colour_manual(values=c("cornflower blue", "red3", "black"))+
  theme(plot.title=element_text(size=8,face="bold"))+
...
ggsave(plot=b, height=5,width=7,dpi=200, device="pdf", filename=paste("pcaovy61.pdf"), limitsize = FALSE)

ggsave(plot=b, height=5,width=7,dpi=200, device="png", filename=paste("pcaovy61.png"), limitsize = FALSE,bg = "transparent")
...
pcdf$type <- c(O,Y,Y,Y,Y,O,O,O,O,Y,Y,Y,Y,O,O,Y,O,O,Y,Y,O,R,R,R,R,B,O,R, B,O,R,O,R,B,O,R,B,O,R,B,O,R,B,O,R,B,O,R,B,O,R)

pcdf <- subset(pcdf, pcdf$type != "B")


#pcdf$group <- c(rep("Old vs Young",25), rep("Pools",14), rep("Single Individuals",12))
pcdf$group <- c(rep(1,25), rep(2,14), rep(3,12))
pcdf$row <- pcdf$type
pcdf$row[pcdf$group > 1] <- substr(pcdf$sample[pcdf$group > 1],1,1)
pcdf$srow <- "Old vs Young"
pcdf$srow[pcdf$group ==2] <- "Pools"
pcdf$srow[pcdf$group ==3] <- "Individuals"
#pcdf <-pcdf[26:51,]

pcdf$srow <- factor(pcdf$srow, levels= c("Old vs Young","Pools", "Individuals"))

p=ggplot(data=pcdf, aes(x=pc1val, y=row, colour=type, label=type)) +
  theme_bw() +
  geom_text()+
  scale_colour_manual(values=c("cornflower blue" , "forest green", "red3"), name="Sample Type", labels=c("Old", "Regenerated","Young"))+
  facet_wrap(~srow, ncol=1, scales="free_y")+
  ylab("Condition") +
  xlab("PC1: 38.9% explained var.") +
  theme(    plot.background = element_rect(fill = "transparent",colour = "transparent")   )

ggsave(plot=p, height=8,width=6,dpi=200, device="png", filename=paste("pc1.png"), limitsize = FALSE,bg = "transparent")
